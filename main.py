...
async def submit_task(req: SubmitRequest, background_tasks: BackgroundTasks):
    ...
    print(f"Starting thread {thread_id} for request: {req.prompt}")
    
    # Save to history for UI
    db.register_thread(thread_id, req.prompt)
    
    # Run synchronous graph invocation in the background to unblock asyncio event loop
    def _run_graph():
        try:
...
async def restart_thread(thread_id: str, background_tasks: BackgroundTasks):
    ...
    # Launch as new job
    return await submit_task(SubmitRequest(prompt=prompt), background_tasks)
