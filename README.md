...
Welcome to OpenColab! This project implements the Phase 1 Local Development Orchestrator for OpenColab. 

It is a robust state machine built with Python, FastAPI, LangGraph, and SQLite. The orchestrator takes a complex user prompt, splits it into dependencies, dispatches them, pauses, and resumes when a worker completes a sub-task.
...
- **Mock Worker**: A separate, standalone script (`mock_worker.py`) that polls the local queue database and simulates asynchronous work.
...
git clone https://github.com/Orizari/OpenColab.git
cd OpenColab
